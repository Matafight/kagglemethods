...
class xgboostRegression_CV(xgboost_base.xgboost_CV):
    def __init__(self,x,y,tunning_params,metric,metric_proba = False,metric_name='rmse',scoring='neg_mean_squared_error',n_jobs=2,save_model=False,processed_data_version_dir = './',if_classification=False):
        """
        初始化相关参数
        ...
        """
        super(xgboostRegression_CV,self).__init__(x,y,tunning_params,metric,metric_proba = metric_proba,metric_name=metric_name,scoring = scoring,n_jobs=n_jobs,save_model=save_model,processed_data_version_dir=processed_data_version_dir,if_classification = if_classification)
